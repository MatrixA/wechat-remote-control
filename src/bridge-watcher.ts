...
function extractTextsFromNewLines(lines: string[]): string[] {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      if (!msg || !msg.stop_reason) continue; // null/undefined = still streaming
...
interface SessionWatch {
  sessionId: string;
  filePath: string;
  fileOffset: number;     // bytes read so far — only push content after this
  pendingBuffer: string;  // incomplete partial line from last read
  watcher: FSWatcher | null;
  debounceTimer: NodeJS.Timeout | null;
  fallbackPollTimer: NodeJS.Timeout | null; // used when fs.watch is unavailable
}
...
export function createBridgeWatcher(
  account: AccountData,
  ...
  let paused = false;
  let bridgePollTimer: NodeJS.Timeout | null = null;
  let currentBridgeSessionId: string | undefined;
  ...
  function stopSessionWatch(): void {
    ...
    if (sessionWatch.debounceTimer) clearTimeout(sessionWatch.debounceTimer);
    if (sessionWatch.fallbackPollTimer) clearInterval(sessionWatch.fallbackPollTimer);
    sessionWatch = null;
  }

  function startSessionWatch(sessionId: string, cwd: string): void {
    if (sessionWatch?.sessionId === sessionId) return; // already on this session
    stopSessionWatch();
  ...
  function processNewContent(): void {
    if (!sessionWatch || paused) return;

    try {
      const stat = statSync(sessionWatch.filePath);
      if (stat.size <= sessionWatch.fileOffset) return;
