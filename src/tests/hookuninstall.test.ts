...
function attachWrite(cfg: any, kind: 'claude' | 'codex', skillDir = SKILL_DIR): any {
  ...
  for (const [event, arg] of Object.entries(hookEventsFor(kind))) {
    ...
    for (const g of groups) {
      g.hooks = (g.hooks ?? []).filter(
        (h: any) => !String(h?.command ?? '').includes('wechat-remote-control/hook.py'),
      );
    }
    hooks[event] = groups.filter((g: any) => g.hooks?.length);
...
test('preserves unrelated top-level settings', () => {
  const cfg: any = { model: 'opus', permissions: { allow: ['Bash'] } };
