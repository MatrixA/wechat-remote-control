...
function cut(from: string, to: string): string {
 ...
 */
function makeProbe(paneWorking: boolean, entries: CodexEntry[]) {
  const calls: ProbeCalls = { paneChecks: 0, rolloutReads: 0, claudeProbes: 0 };
  const factory = new Function(
    'paneShowsWorking', 'getAgent', 'claudeProbe',
    cut('function completionProbe', '// ── Inject-time turn watchdog')
      .replace(/\/\*\*[\s\S]*?\*\/\s*$/, '') + '\nreturn { completionProbe };',
  );
  const { completionProbe } = factory(
    () => { calls.paneChecks++; return paneWorking; },
    () => ({
  ...
  ) as { completionProbe: (k: string, p: string | null, i: string, t: string) => () => unknown };
  return { calls, completionProbe };
}

const userMsg = (message: string): CodexEntry =>
  ({ type: 'event_msg', payload: { type: 'user_message', message } });
const agentMsg = (message: string, phase?: string): CodexEntry =>
  ({ type: 'event_msg', payload: { type: 'agent_message', message, phase } });
const taskComplete = (last_agent_message: string): CodexEntry =>
...
// ── armTurnWatchdog wired to the REAL armOrphanPoll ──
//
// The unit above only proves the probe decides correctly. What actually broke in
// production was that nothing ever *called* a probe, so these exercise the wiring:
// arm at injection, resolve with no hook involved, and say so out loud.
...
function makeWatchdog(opts: { paneWorking: () => boolean; entries: CodexEntry[] }) {
  const calls: WdCalls = { pushed: [], sentText: [], warned: [], abandoned: [] };
  const paneShowsWorking = () => opts.paneWorking();
  const probeFactory = new Function(
    'paneShowsWorking', 'getAgent', 'claudeProbe',
    cut('function completionProbe', '// ── Inject-time turn watchdog')
      .replace(/\/\*\*[\s\S]*?\*\/\s*$/, '') + '\nreturn { completionProbe };',
  );
  const { completionProbe } = probeFactory(
    paneShowsWorking,
    () => ({ parseRollout: () => opts.entries, responseToInjected: codexResponseToInjected }),
    () => () => null,
  );
  const pollFactory = new Function(
    'pushResponse', 'scheduleInject', 'abandonTurn', 'logger',
    cut('const ORPHAN_MAX_EXTENSIONS', '/** Standard Claude transcript probe')
      + '\nreturn { armOrphanPoll };',
  );
  const { armOrphanPoll } = pollFactory(
    (s: any, text: string) => { calls.pushed.push(text); s.lastInjectedText = null; },
    () => {},
    (s: any, reason: string) => { calls.abandoned.push(reason); s.lastInjectedText = null; },
    { info: () => {}, warn: (m: string) => { calls.warned.push(m); }, debug: () => {} },
  );
  const wdFactory = new Function(
    'transport', 'lastTarget', 'tmuxTargetFor', 'sessionKind', 'completionProbe',
    'armOrphanPoll', 'paneShowsWorking', 'logger',
    cut('const WATCHDOG_GRACE_MS', '// Short: the deadline only matters')
      + '\nreturn { armTurnWatchdog, WATCHDOG_GRACE_MS };',
  );
  const api = wdFactory(
    { sendText: (_t: string, text: string) => { calls.sentText.push(text); return Promise.resolve(); } },
    'fallback-target', () => '%1', () => 'codex', completionProbe,
    armOrphanPoll, paneShowsWorking,
    { info: () => {}, warn: (m: string) => { calls.warned.push(m); }, debug: () => {} },
  ) as { armTurnWatchdog: Function; WATCHDOG_GRACE_MS: number };
  return { calls, ...api };
}

const sess = (patch: Record<string, unknown> = {}) => ({
...
test('watchdog waits out a long working turn instead of abandoning it', (t) => {
  t.mock.timers.enable({ apis: ['setTimeout', 'Date'] });
  let working = true;
  const { calls, armTurnWatchdog, WATCHDOG_GRACE_MS } =
    makeWatchdog({ paneWorking: () => working, entries: MID_TURN });
  ...
  const step = (upTo: number) => { while (Date.now() < upTo) t.mock.timers.tick(5_000); };
...
test('watchdog un-wedges a turn whose answer never reads as final', (t) => {
  t.mock.timers.enable({ apis: ['setTimeout', 'Date'] });
  const { calls, armTurnWatchdog, WATCHDOG_GRACE_MS } =
    makeWatchdog({ paneWorking: () => false, entries: MID_TURN });
  const s = sess();
  armTurnWatchdog(s, { kind: 'codex' }, 'MSG');
  const step = (upTo: number) => { while (Date.now() < upTo) t.mock.timers.tick(5_000); };
