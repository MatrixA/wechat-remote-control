...
let paneText = '';
let paneAlive = true;
const paneFactory = new Function(
  'paneExists', 'stripAnsi', 'capturePaneContent',
  extract('function paneShowsWorking', 'function tryAutoConfirmCompaction')
    .replace(/\/\*\*[\s\S]*$/, '') + '\nreturn { paneShowsWorking };'
);
const { paneShowsWorking } = paneFactory(
  () => paneAlive, (s: string) => s, () => paneText,
) as { paneShowsWorking: (target: string) => boolean };

const onPane = (screen: string, alive = true) => {
  paneText = screen;
  paneAlive = alive;
...
const bodyFactory = new Function(
  extract('function fmtDur', 'function statusButtons') + '\nreturn { statusBody };'
);
const { statusBody } = bodyFactory() as { statusBody: (sess: any) => string };
...
// ── armOrphanPoll: object probes, custom abandon reason, extension cap ──

interface PollCalls {
  pushed: Array<{ text: string; opts: any }>;
  abandoned: string[];
  scheduled: number;
}

function makePoll() {
  const calls: PollCalls = { pushed: [], abandoned: [], scheduled: 0 };
  const sess: any = { name: 't', lastInjectedText: 'MSG', orphanPollText: null };
  const factory = new Function(
    'pushResponse', 'scheduleInject', 'abandonTurn', 'logger',
    extract('const ORPHAN_MAX_EXTENSIONS', '/** Standard Claude transcript probe')
      + '\nreturn { armOrphanPoll, ORPHAN_MAX_EXTENSIONS };'
  );
  const api = factory(
    (s: any, text: string, opts: any) => { calls.pushed.push({ text, opts }); s.lastInjectedText = null; },
    () => { calls.scheduled++; },
    (s: any, reason: string) => { calls.abandoned.push(reason); s.lastInjectedText = null; },
    { info: () => {}, warn: () => {} },
  ) as { armOrphanPoll: Function; ORPHAN_MAX_EXTENSIONS: number };
...
test('armOrphanPoll extends while stillInFlight, then caps and abandons', (t) => {
  ...
  const step = (upToMs: number) => {
    while (Date.now() < upToMs) t.mock.timers.tick(5_000);
  };
