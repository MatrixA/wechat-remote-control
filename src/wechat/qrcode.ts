...
class QrCode {
  ...
  private readonly modules: Bit[][];
  private readonly isFunction: boolean[][];

  constructor(
    readonly version: number,
    private readonly eccOrdinal: number,
    dataCodewords: number[],
    mask: number,
  ) {
    this.size = version * 4 + 17;
    const size = this.size;
    this.modules = Array.from({ length: size }, () => new Array<Bit>(size).fill(false));
...
function encodeBytesToCodewords(data: number[], version: number, eccOrdinal: number): number[] {
  const bits: number[] = [];
 ...
 */
export function renderTerminalQr(text: string, opts: { margin?: number } = {}): string {
