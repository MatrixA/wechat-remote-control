...
export function handlePermission(ctx, args) {
    if (!args) {
    ...
    ctx.updateSession({ permissionMode: mode });
...
export function handleVersion() {
    try {
        ...
        return { reply: `wechat-bridge v${version}`, handled: true };
    }
    catch {
        return { reply: 'wechat-bridge (version unknown)', handled: true };
    }
}
