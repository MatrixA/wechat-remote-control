 ...
 * odd one out.
 */
export function isWrcHookCommand(command, skillDir) {
    const cmd = String(command ?? '');
    if (!cmd)
        return false;
    if (cmd.includes(HOOK_MARK))
        return true;
    return !!skillDir && cmd.includes(join(skillDir, 'hook.py'));
}
/** Which config file holds this agent's hooks, given its config dir. */
export function hookConfigFileFor(kind, configDir) {
 ...
 */
export function mergeWrcHooks(cfg, skillDir, kind) {
    ...
    for (const [event, arg] of Object.entries(hookEventsFor(kind))) {
        ...
        for (const g of groups) {
            ...
            const filtered = inner.filter((h) => !isWrcHookCommand(h?.command, skillDir));
...
export function wrcStatusLine(skillDir) {
