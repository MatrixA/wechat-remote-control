...
function assistantTextFromResponseItem(e) {
    ...
    if (Array.isArray(content)) {
        const text = content
            .filter((b) => b && (b.type === 'output_text' || b.type === 'text') && typeof b.text === 'string')
            .map((b) => b.text)
            .join('');
        return text || null;
    }
...
export function codexLatestUserMessage(entries) {
 ...
 */
export function codexLastAgentMessage(entries) {
    for (let i = entries.length - 1; i >= 0; i--) {
        const e = entries[i];
 ...
 */
export function codexResponseToInjected(entries, injectedText) {
    if (!injectedText)
        return null;
    let userIdx = -1;
    for (let i = entries.length - 1; i >= 0; i--) {
    ...
    for (let i = userIdx + 1; i < entries.length; i++) {
        ...
        if (e.type === 'event_msg' && payloadType(e) === 'agent_message') {
            ...
            if (typeof m === 'string' && m.trim()) {
                lastAgentText = m;
                if (e.payload?.phase === 'final_answer')
                    lastCompleteText = m;
            }
        }
    ...
    if (lastCompleteText)
        return { text: lastCompleteText, complete: true };
    if (lastAgentText)
        return { text: lastAgentText, complete: false };
    return null;
}
 ...
 */
export function codexContextReplay(entries, rounds = 3) {
    const pairs = [];
