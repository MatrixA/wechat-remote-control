...
export async function createTransport(name) {
    if (name === 'telegram') {
 ...
 *   4. default 'wechat'
 */
export function resolveTransportName(argv = process.argv.slice(2)) {
    const env = (process.env.WCC_TRANSPORT || '').toLowerCase();
    if (env === 'telegram' || env === 'wechat')
        return env;
    if (argv.includes('--telegram'))
        return 'telegram';
    if (argv.includes('--wechat'))
        return 'wechat';
    ...
    if (existsSync(TELEGRAM_ACCOUNT) && !hasWeChatAccount())
        return 'telegram';
    return 'wechat';
}
