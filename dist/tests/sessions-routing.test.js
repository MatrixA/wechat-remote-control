...
import { sessionKeyFor, newSessionState, resetSidCounter, resolveSessionForHook, resolveSessionForInbound, persistableState, migrateLegacyIlink, planTopicSync, } from '../sessions.js';
...
test('hook routing: same-project fallback needs an in-flight Claude turn', () => {
    ...
    assert.strictEqual(resolveSessionForHook(reg({ active: 'beta' }), [idle], { transcript_path: '/t/p-alpha/new.jsonl' }), null);
...
test('migrateLegacyIlink attaches a legacy in-flight turn to the active session key', () => {
    const out = migrateLegacyIlink({ lastInjectedText: 'old turn', lastInjectedTranscript: '/t/p-alpha/a.jsonl', target: '42' }, reg());
