...
const { stuckSuffix } = new Function('fmtDur', cut('// A turn that has been in flight this long', 'function formatTmuxList')
    + '\nreturn { stuckSuffix };')(fmtDur);
// ── fmtDur ───────────────────────────────────────────────────────────
...
function makeReset() {
    const calls = [];
    const factory = new Function('cancelQuiz', 'cancelPending', 'stopTurnStatus', 'persistStates', 'scheduleInject', 'logger', cut('function resetSessionState', '// ── Send with retry')
        + '\nreturn { resetSessionState };');
    const { resetSessionState } = factory(() => calls.push('cancelQuiz'), () => calls.push('cancelPending'), () => calls.push('stopTurnStatus'), () => calls.push('persistStates'), () => calls.push('scheduleInject'), { warn: () => { }, info: () => { }, debug: () => { } });
