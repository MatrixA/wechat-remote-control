...
const factory = new Function('execFileSync', 'logger', cut('function paneExists', '// ── Quiz (AskUserQuestion) helpers')
    + '\n' + cut('function quizNorm', '/**\n * Is `question` the quiz')
    + '\nreturn { classifyComposer, composerRegion };');
const { classifyComposer, composerRegion } = factory(() => '', { warn: () => { }, info: () => { }, debug: () => { } });
// ── Fixtures ─────────────────────────────────────────────────────────
...
test('classifyComposer: multi-line text left in the composer reads stuck', () => {
    assert.strictEqual(classifyComposer(CODEX_MULTILINE_STUCK, 'line one alpha\nline two beta\nline three gamma', 'codex'), 'stuck');
});
