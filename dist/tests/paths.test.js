...
test('encodeCwd is ASCII-only — non-ASCII chars each become a dash', () => {
    ...
    const detectPyEncode = (cwd) => [...cwd].map((c) => (/[A-Za-z0-9]/.test(c) && c.codePointAt(0) < 128) || c === '-' ? c : '-').join('');
...
test('isSameProjectDir: same project dir → true, foreign project → false', () => {
    ...
    assert.strictEqual(isSameProjectDir(`${proj}/a.jsonl`, '/Users/me/.claude/projects/-Users-me-other/c.jsonl'), false);
