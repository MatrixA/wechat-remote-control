...
let paneText = '';
const factory = new Function('capturePaneContent', 'stripAnsi', 'logger', src.slice(start, end) + '\nreturn { quizOnScreen };');
const { quizOnScreen } = factory(() => paneText, (s) => s, { debug: () => { } });
// attempt=1 skips the 500ms blocking redraw-retry — these are static fixtures.
const onScreen = (screen, question) => {
    paneText = screen;
